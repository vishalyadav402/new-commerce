...
const Page = () => {
  ...
  const handleSubcategoryClick = (subcategorySlug) => {
    ...
    router.push(`/${url_category}/${subcategorySlug}`,undefined, { shallow: true });
  };
  // ✅ Handle Category Click
  const handleCategoryClick = (categorySlug) => {
    setUrlCategory(categorySlug);
    setUrlSubcategory(null);
    router.push(`/${categorySlug}`, undefined, { shallow: true });
  };
      ...
      <div className="relative flex items-center md:mb-2 h-10 bg-white">
        ...
        <div className="grid grid-cols-5 gap-0">
          ...
          <div className="col-span-4 max-h-[75vh] overflow-auto no-scrollbar">
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-1 p-1 md:gap-2 min-h-[75vh] bg-gray-100 md:p-4 p:2 rounded">
              {loading ? (
                [...Array(12)].map((_, i) => (
                ...
                <p className="text-sm">Please try a different category or check back later.</p>
              </div>
              )}
            </div>
          </div>
